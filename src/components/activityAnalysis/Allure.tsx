...
export default function Allure({ activity }: AllureProps) {  const [loading, setLoading] = useState(false);
  ...
  const { user } = useAuth();
  const supabase = createClientComponentClient();  // Fonction pour formater les secondes en min:sec
  ...
  if (loading) {
    ...
    <div>
      ...
      )}        {/* Overlay Modal pour afficher le graphique en grand - Centré sur la page analyse */}
      {isModalOpen && (
        <>
